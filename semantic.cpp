    ...
    switch (node->type)
    {
        ...
        case ASTNodeType::DEC_FUNC:
            ...
            else
            {
                ...
                node->children[1]->symbol->param_count = node->children[2]->children.size();
        ...
        case ASTNodeType::PARAM:
            ...
            {
                cout << "Parameter " << node->children[1]->symbol->get_text() << " declared" << endl;
...
bool checkVectorElements(AST* node, int data_type){
    for (AST* child : node->children) {
        if(DatatypeCompatible(child->symbol->get_data_type(), data_type) == 0){
            cout << "Element " << child->symbol->get_text() << " has incompatible data type" << endl;
            return false;
        }
    }
    return true;
    ...
    switch (node_type) {
        ...
        case OR: {
            ...
            if (left.second == DATA_TYPE_BOOL && right.second == DATA_TYPE_BOOL) {
                return std::make_pair(0, DATA_TYPE_BOOL); // Logical operations return bool
            }
        ...
        case READ: {
            ...
            return std::make_pair(0, 0); // Return type is void (no return value for READ)
        }
        ...
        case FUNCALL: {
            ...
            if (fun_symbol->param_count != node->children[1]->children.size()) {
                fprintf(stderr, "Invalid argument number for function call.\n");
                exit(4);
            }

            std::pair<int, int> arg_list = type_infer(node->children[1]);
            return std::make_pair(SYMBOL_FUNC, fun_symbol->data_type); // Return function type
        }
